 ...
 */
public class SequenceTableRowData {

	private ObservableValue<Integer> id;
    private SimpleStringProperty sequence;
    private AptamerFamilyAnalysisRootController aptamerFamilyAnalysisRootController;
    ...
    public SequenceTableRowData(int id, String sequence, AptamerFamilyAnalysisRootController afarc) {
    	
    	this.id = new SimpleObjectProperty<Integer>(id);
    	this.sequence = new SimpleStringProperty(sequence);
    	this.aptamerFamilyAnalysisRootController = afarc;
	...
	public ObservableValue<Number> getCount(SelectionCycle sc) {
		...
		if (this.aptamerFamilyAnalysisRootController.getCmpRadioButton().isSelected()) {
			count = (sc.getAptamerCardinality(id.getValue()) / (double) sc.getSize()) * 1000000;
		}

		else if (this.aptamerFamilyAnalysisRootController.getRawCountsRadionButton().isSelected()) {
	...
	/**
	 * Returns the frequency of that aptamer the cluster <code>sc</code>
	 * 
	 * @param sc
	 * @return
	 */
	public ObservableValue<Number> getClusterAbundance(SelectionCycle sc) {
